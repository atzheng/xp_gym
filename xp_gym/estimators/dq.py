...
@struct.dataclass
class AvgRewardLSTDDQEstimatorState(EstimatorState):
    """
    State for DQ-based value difference estimator using LSTD.

    This estimator tracks state transitions and rewards to compute
    treatment effects using the Dq approach with LSTD equations.
    """
...
@struct.dataclass
class AvgRewardLSTDDQEstimator(Estimator):
    """
    DQ-based value difference estimator using LSTD.

    This estimator implements the Dq method for treatment effect estimation
    ...
    def reset(self, rng, env, env_params, design):
        ...
        return AvgRewardLSTDDQEstimatorState(
    ...
    def update(
        ...
        """
        Update Dq estimator with new observation.
        ...
        current_state = available_cars_per_zone(
            ...
            jnp.float32
        )  # Convert to float for LSTD operations
        ...
        def update_with_transition():
            ...
            new_sum_ss_rs = (
                state.sum_ss_rs + state.prev_state * state.prev_reward
            )
        ...
        (
            ...
            state.has_prev_state, update_with_transition, no_update
        )
        ...
        def update_treatment_groups():
            treated_update = jnp.where(
                state.prev_action > 0.5,
                state.prev_state,
                jnp.zeros_like(state.prev_state),
            )
            control_update = jnp.where(
                state.prev_action <= 0.5,
                state.prev_state,
                jnp.zeros_like(state.prev_state),
            )
            ...
            return (
                treated_update,
                control_update,
                count_treated_update,
                count_control_update,
            )

        def no_treatment_update():
            return (
                jnp.zeros_like(current_state),
                jnp.zeros_like(current_state),
                0,
                0,
            )
        ...
        (
            ...
            state.has_prev_state, update_treatment_groups, no_treatment_update
        )
    ...
    def estimate(self, env, env_params, design, state: AvgRewardLSTDDQEstimatorState):
        ...
        def compute_estimate():
            ...
            mean_states_treated = state.states_treated / jnp.maximum(
                state.count_treated, 1
            )
            mean_states_control = state.states_control / jnp.maximum(
                state.count_control, 1
            )
        ...
        return jax.lax.cond(
            sufficient_data, compute_estimate, insufficient_data
        )
